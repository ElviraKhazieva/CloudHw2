...
async def after_server_start(app, loop):
    print(f"Application listening at port {os.environ['PORT']}")
    global config
    config = {
...
def get_driver():
    ...
    driver_config = ydb.DriverConfig(endpoint, path, credentials=creds)
...
def save_image_information_to_db(original_id, face_id):
    rand = random.Random()
    query = f"""
...
def process_message(message):
    ...
    bottom = sorted_y[1]
    rand = random.Random()
    face_id = f"{body['object_key'].removesuffix('.jpg')}_{rand.randint(0, 1000)}.jpg"
    ...
    save_image_information_to_db(body['object_key'], face_id)
